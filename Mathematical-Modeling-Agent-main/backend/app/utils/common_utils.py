...
def md_2_docx(task_id: str):
    ...
    md_path = os.path.join(work_dir, "paper.md")
    docx_path = os.path.join(work_dir, "paper.docx")
...
def md_2_pdf(task_id: str):
    work_dir = get_work_dir(task_id)
    md_path = os.path.join(work_dir, "paper.md")
    pdf_path = os.path.join(work_dir, "paper.pdf")

    engines = ["xelatex", "tectonic", "wkhtmltopdf", "pdflatex"]
    ...
    for engine in engines:
        try:
            ...
            logger.info(f"PDF转换完成: {pdf_path} (engine={engine})")
            return pdf_path
        except Exception as exc:
            last_error = exc
            logger.warning(f"PDF转换失败(engine={engine}): {exc}")

    raise RuntimeError(f"无法将 Markdown 转换为 PDF: {last_error}")


def tex_2_pdf(task_id: str):
    work_dir = get_work_dir(task_id)
    tex_path = os.path.join(work_dir, "paper.tex")
    pdf_path = os.path.join(work_dir, "paper.pdf")

    if not os.path.exists(tex_path):
        raise FileNotFoundError(f"TeX主文件不存在: {tex_path}")
        ...
        try:
            for _ in range(2):
                ...
                if result.returncode != 0:
                    ...
                    raise RuntimeError(f"{engine} 编译失败:\n{output}")

            if not os.path.exists(pdf_path):
                raise RuntimeError(f"{engine} 编译完成但未找到输出文件: {pdf_path}")

            logger.info(f"TeX编译完成: {pdf_path} (engine={engine})")
            return pdf_path
        except Exception as exc:
            last_error = exc
            logger.warning(f"TeX编译失败(engine={engine}): {exc}")

    raise RuntimeError(f"无法将 TeX 转换为 PDF: {last_error}")


def export_paper_pdf(task_id: str):
    try:
        return tex_2_pdf(task_id)
    except Exception as tex_exc:
        logger.warning(f"TeX主文件编译失败，回退到 Markdown 转 PDF: {tex_exc}")
        return md_2_pdf(task_id)
