...
class E2BCodeInterpreter(BaseCodeInterpreter):
    ...
    async def _pre_execute_code(self):
        init_code = self.get_matplotlib_font_setup_code()
    ...
    async def execute_code(self, code: str) -> tuple[str, bool, str]:
        ...
        prepared_code = self.prepare_code_for_execution(code)
        logger.info(f"执行代码: {prepared_code}")
        self.notebook_serializer.add_code_cell_to_notebook(prepared_code)
        ...
        execution = await self.sbx.run_code(prepared_code)  # 返回 Execution 对象
