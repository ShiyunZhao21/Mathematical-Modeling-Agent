...
from app.utils.common_utils import create_work_dir, get_config_template, md_2_docx, md_2_pdf
...
class MathModelWorkFlow(WorkFlow):
    ...
    async def execute(self, problem: Problem):
        ...
        await task_store.update_task(
            ...
            status="running",
            stage="coordinator",
            message="Coordinator is analyzing the problem",
        )
        ...
        await task_store.update_task(
            self.task_id,
            stage="modeler",
            message="Modeler is building the modeling plan",
        )
        await task_store.publish_message(
            self.task_id,
        ...
        await task_store.update_task(
            self.task_id,
            stage="interpreter",
            message="Preparing local code interpreter",
        )
        await task_store.publish_message(
            self.task_id,
        ...
        try:
            ...
            await task_store.update_task(
                self.task_id,
                stage="coder",
                message="Coder is solving modeling subtasks",
            )
            await task_store.publish_message(
                self.task_id,
            ...
            for key, value in solution_flows.items():
                await task_store.update_task(
                    self.task_id,
                    stage=f"coder:{key}",
                    message=f"Coder is solving {key}",
                )
                await task_store.publish_message(
                    self.task_id,
                ...
                await task_store.update_task(
                    self.task_id,
                    stage=f"writer:{key}",
                    message=f"Writer is drafting {key}",
                )
                await task_store.publish_message(
                    self.task_id,
